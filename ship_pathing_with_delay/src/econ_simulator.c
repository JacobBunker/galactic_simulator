...
Vec2 GetBodyPositionAtTime(SimulationState *sim, float time, int body_index) {
	Vec2 local = cv2((sim->g.bodies[body_index].orbit_shape.x * sinf((time + sim->g.bodies[body_index].orbit_offset) * sim->g.bodies[body_index].orbit_speed)),
					 (sim->g.bodies[body_index].orbit_shape.y * cosf((time + sim->g.bodies[body_index].orbit_offset) * sim->g.bodies[body_index].orbit_speed)));
...
int RecursiveStepBody(SimulationState *sim, float time, int current, float parentX, float parentY, int level) {
	int leap = 1;
	sim->g.bodies[current].pos.x = parentX + (sim->g.bodies[current].orbit_shape.x * sinf((time + sim->g.bodies[current].orbit_offset) * sim->g.bodies[current].orbit_speed));
	sim->g.bodies[current].pos.y = parentY + (sim->g.bodies[current].orbit_shape.y * cosf((time + sim->g.bodies[current].orbit_offset) * sim->g.bodies[current].orbit_speed));
	for(int i = 0; i < sim->g.bodies[current].num_children; ++i) {
