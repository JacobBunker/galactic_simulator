...
int CheckRocksInSquare(SimulationState *sim, Tile *t) {
	int sum = 0;
	for(int i = 0; i < sim->g.bodies_num; ++i) {
		if(Check_In_Square(t->shape, sim->g.bodies[i].pos.x, sim->g.bodies[i].pos.y)) {
			sum++;
		}
	}
